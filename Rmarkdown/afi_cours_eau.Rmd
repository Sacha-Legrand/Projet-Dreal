...
init.values<-ifa.init.random(aci_data,2)
fit<-ifa.em(aci_data,c(2,2),it=50,eps=0.0001,init.values)
pred = as.data.frame(ifa.predict(scale(aci_data),fit))

B <- cbind(aci_t, pred)


xts_B = xts(B[, c("V1", "V2")], order.by = B[,"t"])

#xts_B = xts(B[, c("V1", "V2", "V3")], order.by = B[,"t"])
```
...
for(i in 1:ncol(aci_data)){
  ...
  B[paste0("comp1_", label_i)]=fit$H[i,1]*pred[,1]
  B[paste0("comp2_", label_i)]=fit$H[i,2]*pred[,2]
  #B[paste0("comp3_", label_i)]=fit$H[i,3]*pred[,3]
...
```{r}
dygraph(xts_B_comp[, c("comp1_812", "comp2_812")], 
